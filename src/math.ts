...
export const mat4 = {
  ...
  rotationX(rad: number): Mat4 {
    ...
    const m = mat4.identity();
    // Column-major: col1 = [0, c, s, 0], col2 = [0, -s, c, 0]
    m[5] = c;  m[6] = s;
    m[9] = -s; m[10] = c;
    return m;
  },
  ...
  rotationY(rad: number): Mat4 {
    const c = Math.cos(rad), s = Math.sin(rad);
    const m = mat4.identity();
    // Column-major: col0 = [c, 0, -s, 0], col2 = [s, 0, c, 0]
    m[0] = c;  m[2] = s;   // col 0: row0=c, row2=-s  → indices 0, 2
    m[8] = -s; m[10] = c;  // col 2: row0=s, row2=c   → indices 8, 10
    return m;
  },
  ...
  perspective(fovyRad: number, aspect: number, near: number, far: number): Mat4 {
    ...
    m[0]  = f / aspect;
    m[5]  = f;
    m[10] = far / (near - far);        // WebGPU depth range [0, 1]
    m[11] = -1;
    m[14] = (far * near) / (near - far);
    return m;
  },

  // FIX: column-major storage — each basis vector goes DOWN a column, not across a row
  lookAt(eye: Vec3, target: Vec3, up: Vec3): Mat4 {
    const z = vec3.normalize(vec3.sub(eye, target)); // backward (away from target)
    ...
    m[10] = z[2];
    m[11] = 0;
    // Column 3 — translation (dot products for view-space position)
    m[12] = -vec3.dot(x, eye);
    m[13] = -vec3.dot(y, eye);
    m[14] = -vec3.dot(z, eye);
    m[15] = 1;
